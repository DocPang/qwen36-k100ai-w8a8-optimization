...
## Current release: R269

For a fresh deployment, reproduce the newest accepted cumulative release first:
...
Expected release magnitude on the validated stack is about **106 tok/s on GPU1**, with **107.46 tok/s median** observed on GPU7. The exact MTP throughput remains workload-dependent.

The historical reproduction procedure below is intentionally retained because it documents the earlier R180 / R184 / R202 stages and is useful for regression testing or understanding where each optimization came from.
...
9. Start scripts/serve_mtp3.sh on one free GPU to reproduce the historical R184 milestone.
10. Warm up, then run the same fixed benchmark. A conservative historical R184 expected result is around 85 tok/s; speculative acceptance can make some workloads substantially faster.
...
16. For the current R269 release, also run `scripts/quickstart_r269.sh` and `scripts/benchmark_fixed_512.py`, then compare with `docs/R269_RELEASE_NOTES.md` and `results/RESULTS.md`.
17. Save all changed files and benchmark output. If any source edit is made, run `python -m py_compile` on all public `patches/*/sitecustomize.py` files before serving.
```
