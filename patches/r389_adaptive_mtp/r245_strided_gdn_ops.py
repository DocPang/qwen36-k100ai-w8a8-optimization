...
def fused_sigmoid_gating_delta_rule_update_kernel(
    ...
    p_q = q + bos * stride_qt + i_h * stride_qh + o_k
    p_k = k + bos * stride_kt + i_h * stride_kh + o_k
    p_v = v + bos * stride_vt + i_hv * stride_vh + o_v

    p_A_log = A_log + i_hv
    if not IS_KDA:
        lane = i_hv % (HV // H)
        p_a = a + bos * stride_at + i_h * stride_ah + lane * stride_al
    ...
    lane = i_hv % (HV // H)
    p_b = b + bos * stride_bt + i_h * stride_bh + lane * stride_bl
...
def fused_sigmoid_gating_delta_rule_update(
    ...
    ratio = HV // H
    if a.ndim == 2:
        stride_at, stride_ah, stride_al = a.stride(0), a.stride(1) * ratio, a.stride(1)
    else:
        stride_at, stride_ah, stride_al = a.stride(0), a.stride(1), a.stride(2)
    if b.ndim == 2:
        stride_bt, stride_bh, stride_bl = b.stride(0), b.stride(1) * ratio, b.stride(1)
    else:
        stride_bt, stride_bh, stride_bl = b.stride(0), b.stride(1), b.stride(2)
