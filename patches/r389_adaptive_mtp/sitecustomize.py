...
if _LINEAR_ENABLED:
    ...
    _TRITON_LINEAR_CONFIGS_BY_M = {
        ...
        # R299: MTP draft full-attention QKV. gfx928 exact tuning cut
        # ~63.47us -> ~38.38us (1.654x) for M=1.
        (1, 2048, 9216): {
            "bm": 8, "bn": 8, "bk": 2048, "warps": 2, "waves": 2,
            "stages": 1, "mi": 16, "kp": 1, "mlf": 1,
            "lat": "none", "hint": "local-prefetch",
        },
        ...
        # R296: MTP FC and full-attention O projection. The same M=1 shape is
        # hit at least twice per serial draft forward. 49.30us -> 22.15us.
        (1, 4096, 2048): {
            "bm": 8, "bn": 8, "bk": 2048, "warps": 1, "waves": 1,
            "stages": 1, "mi": 16, "kp": 2, "mlf": 1,
            "lat": "none", "hint": "local-prefetch",
        },
        # R297: shared-expert gate+up. 17.85us -> 12.22us.
        (1, 2048, 1024): {
            "bm": 8, "bn": 8, "bk": 1024, "warps": 2, "waves": 1,
            "stages": 2, "mi": 16, "kp": 2, "mlf": 1,
            "lat": "none", "hint": "none",
        },
        # R298: shared-expert down projection was not shape-specialized in
        # R269. Vendor heuristic 12.81us -> 10.38us, bitwise exact.
        (1, 512, 2048): {
            "bm": 8, "bn": 8, "bk": 512, "warps": 1, "waves": 2,
            "stages": 1, "mi": 16, "kp": 1, "mlf": 1,
            "lat": "none", "hint": "none",
        },
        (1, 2048, 64): {
            ...
            if triton_config is not None:
                ...
                if "kp" in triton_config:
                    ...
                    _scaled_mm_kernel[grid](
                        ...
                        num_stages=triton_config.get("stages", 2),
...
# R304: Qwen3.6's built-in MTP path can ignore SpeculativeConfig.max_model_len
# when HCU GPUModelRunner has no standalone draft_model_config. Force only the
# effective drafter limit so long-context requests can enter the upstream
# no-draft fallback while target max_model_len remains unchanged.
if int(os.getenv("K100_R304_DRAFTER_MAX_MODEL_LEN", "0")) > 0:
    try:
        import r304_force_drafter_cutoff as _k100_r304_force_drafter_cutoff
        _k100_r304_force_drafter_cutoff.install()
    except Exception as _r304_exc:
        print(
            f"[K100 R304 adaptive MTP] disabled: {_r304_exc!r}",
            flush=True,
        )

# R305: AsyncScheduler normally seeds speculative placeholders after every
# decode step.  Above the long-context cutoff, suppress only those future
# placeholders so the next target step can become a true M=1 decode.
if int(os.getenv("K100_R305_SPEC_CUTOFF", "0")) > 0:
    try:
        import r305_adaptive_scheduler as _k100_r305_adaptive_scheduler
        _k100_r305_adaptive_scheduler.install()
    except Exception as _r305_exc:
        print(
            f"[K100 R305 adaptive scheduler] disabled: {_r305_exc!r}",
            flush=True,
        )
